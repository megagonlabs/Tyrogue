...
class PAWS_QqpProcessor(DataProcessor):
...
glue_tasks = ["cola", "mnli", "mrpc", "sst-2", "sst-2-imbalance", "sts-b", "qqp", "paws-qqp", "qnli", "rte", "wnli"]
...
class ngProcessor(DataProcessor):
    def get_labels(self):
        # labels = ["BACKGROUND", "OBJECTIVE", "METHODS", "RESULTS", "CONCLUSIONS"]
        labels = list(map(str,range(6)))
        # labels = list(range(20))
        return labels
    def get_train_examples(self, data_dir):
        """See base class."""""
        df_pkl = pd.read_pickle(os.path.join(data_dir, "train.pkl"))
        sentences = list(df_pkl.index)
        labels = list(df_pkl[1])
        leng = len(sentences)
        lines = []
        for i in range(leng):
            lines.append([sentences[i],str(labels[i])])
        return self._create_examples(lines, "train")

    def get_dev_examples(self, data_dir):
        """See base class."""""
        df_pkl = pd.read_pickle(os.path.join(data_dir, "dev.pkl"))
        sentences = list(df_pkl.index)
        labels = list(df_pkl[1])
        leng = len(sentences)
        lines = []
    ...
    def get_test_examples(self, data_dir):
        """See base class."""""
        df_pkl = pd.read_pickle(os.path.join(data_dir, "test.pkl"))
        sentences = list(df_pkl.index)
        labels = list(df_pkl[1])
        leng = len(sentences)
        lines = []
...
processors = {
    ...
    "mrpc": MrpcProcessor,
    "sst-2": Sst2Processor,
    "sst-2-imbalance": Sst2Processor,
    "sts-b": StsbProcessor,
    "qqp": QqpProcessor,
    "paws-qqp": PAWS_QqpProcessor,
    "qnli": QnliProcessor,
    "rte": RteProcessor,
    "wnli": WnliProcessor,
...
output_modes = {
    ...
    "sst-2-imbalance": "classification",
    "sts-b": "regression",
    "qqp": "classification",
