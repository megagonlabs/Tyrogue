...
#     sup_criterion = nn.CrossEntropyLoss(reduction='none')
    ...
    for _ in train_iterator:
        ...
        for step, batch in enumerate(epoch_iterator):
            ...
            model.train()
######## freezing layers ############
            block = model.bert.encoder
            for layer_ind, layer in enumerate(block.layer):
...
def train_transformer(args, train_dataset, eval_dataset, model, tokenizer, unsup_dataset=None,
    ...
    ##############################
     #, val_acc, val_loss
    global_step, tr_loss = train(args, train_dataset, eval_dataset, model, tokenizer, unsup_dataset)
...
#     if args.do_eval and args.local_rank in [-1, 0]:
    ...
    result, logits = my_evaluate(eval_dataset, args, model)

    # eval_loss = val_loss
    return model, tr_loss, result


def train_transformer_model(args, X_inds, X_val_inds=None, model=None, X_history_inds=None,
